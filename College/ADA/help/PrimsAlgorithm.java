...
public class PrimsAlgorithm{
     ...
     int minKey(int key[],boolean mstSet[], int V) {
        int min = Inetger.MAX_VALUE, minIndex = -1;
         for(int v =o;v<V;v++){
     ...
     void primeMST(int grapgh[][],int V){
        ...
        void printMST(int parent[],int graph[][],int V){
            Ssytem.out.print("Edge \tWeight");
            for(int i =1;i<V;i++){
                Sytem.out.println(parent[i] + "-" + i + "\t" + graph[i][parent[i]]);
            }
             public static vouid main(String args[]{
                prims Algorithm, t = new PrimsAlgorithm();
         ...
         // Print the constructed MSt
            ...
            //Update key value and parent index of the adjacent verices of the picked vertex
            //Consider only those verices which are not yet included in mst
